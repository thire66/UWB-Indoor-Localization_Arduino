...
void DW1000RangingClass::generalStart() {
	...
	_rangingCountPeriod = esp_timer_get_time();
}
...
void DW1000RangingClass::checkForReset() {
	uint32_t curMillis = esp_timer_get_time();
...
void DW1000RangingClass::loop() {
	checkForReset();
	uint32_t time = esp_timer_get_time();  // TODO other name - too close to "timer"
...
void DW1000RangingClass::handle_sentAck() {
    _sentAck = false;
    
    int messageType = detectMessageType(data);
    
    if(messageType != POLL_ACK && messageType != POLL && messageType != RANGE) {
        Serial.print("\nUnexpected message type in handle_sentAck");
        return;
    }
    
    if(_type == ANCHOR) {
        handleAnchorSentAck(messageType);
    }
    else if(_type == TAG) {
        handleTagSentAck(messageType);
    }
}

void DW1000RangingClass::handleAnchorSentAck(int messageType) {
    if(messageType == POLL_ACK) {
        ...
        if (myDistantDevice) {
            ...
            Serial.print("\nNo distant device found for address: ");
			Serial.print(_lastSentToShortAddress[0], HEX);
            Serial.print(_lastSentToShortAddress[1], HEX);
        }
    }
}

void DW1000RangingClass::handleTagSentAck(int messageType) {
    DW1000Time sentTime;
    DW1000.getTransmitTimestamp(sentTime);

    if(messageType == POLL) {
        updatePollTime(sentTime);
    }
    else if(messageType == RANGE) {
        updateRangeTime(sentTime);
    }
}

void DW1000RangingClass::updatePollTime(const DW1000Time& sentTime) {
    if(isBroadcast(_lastSentToShortAddress)) {
        for(uint16_t i = 0; i < _networkDevicesNumber; i++) {
            _networkDevices[i].timePollSent = sentTime;
        }
    } else {
        updateDeviceTime(_lastSentToShortAddress, &DW1000Device::timePollSent, sentTime);
    }
}

void DW1000RangingClass::updateRangeTime(const DW1000Time& sentTime) {
    if(isBroadcast(_lastSentToShortAddress)) {
        for(uint16_t i = 0; i < _networkDevicesNumber; i++) {
            _networkDevices[i].timeRangeSent = sentTime;
        }
    } else {
        updateDeviceTime(_lastSentToShortAddress, &DW1000Device::timeRangeSent, sentTime);
    }
}

void DW1000RangingClass::updateDeviceTime(byte* address, DW1000Time DW1000Device::* timeField, const DW1000Time& sentTime) {
    DW1000Device* myDistantDevice = searchDistantDevice(address);
    if (myDistantDevice) {
        myDistantDevice->*timeField = sentTime;
    } else {
        Serial.print("\nNo distant device found for address: ");
		Serial.print(_lastSentToShortAddress[0], HEX);
		Serial.print(_lastSentToShortAddress[1], HEX);
...
bool DW1000RangingClass::isBroadcast(byte* address) {
    return address[0] == BROADCAST_ADDRESS && address[1] == BROADCAST_ADDRESS;
}
...
//-----handle_receivedAck----------------------------------------------------
//###########################################################################
void DW1000RangingClass::handle_receivedAck() {
    _receivedAck = false;
    ...
    if (_type == ANCHOR) {
        handleAnchorReceivedAck(messageType, data);
    } else if (_type == TAG) {
        handleTagReceivedAck(messageType, data);
    }
}

void DW1000RangingClass::handleAnchorReceivedAck(int messageType, byte* data) {
    switch (messageType) {
        case BLINK:
            handleBlinkMessage(data);
            break;
        case POLL:
            handlePollMessage(data);
            break;
        case RANGE:
            handleRangeMessage(data);
            break;
        default:
            Serial.print("\nUnexpected message type for Anchor");
    }
}

void DW1000RangingClass::handleTagReceivedAck(int messageType, byte* data) {
    switch (messageType) {
        case POLL_ACK:
            handlePollAckMessage(data);
            break;
        case RANGE_REPORT:
            handleRangeReportMessage(data);
            break;
        case RANGE_FAILED:
            handleRangeFailedMessage();
            break;
        default:
            Serial.print("\nUnexpected message type for Tag");
    }
}

void DW1000RangingClass::handleBlinkMessage(byte* data) {
    byte address[8];
    byte shortAddress[2];
...
void DW1000RangingClass::handlePollMessage(byte* data) {
    byte address[2];
...
void DW1000RangingClass::handleRangeMessage(byte* data) {
    byte address[2];
    _globalMac.decodeShortMACFrame(data, address);
...
void DW1000RangingClass::handlePollAckMessage(byte* data) {
    byte address[2];
    _globalMac.decodeShortMACFrame(data, address);
...
void DW1000RangingClass::handleRangeReportMessage(byte* data) {
    float curRange;
...
void DW1000RangingClass::handleRangeFailedMessage() {
    Serial.print("\nRange failed");
